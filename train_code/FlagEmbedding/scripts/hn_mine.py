...
class ModelArgs:
    ...
    lora_path: Optional[str] = field(
        default=None, metadata={"help": "The lora path."}
    )
...
def load_model(model_args: ModelArgs):
    ...
    if model_args.lora_path is not None:
        print(f'loading lora from {model_args.lora_path}')
        model.model = PeftModel.from_pretrained(model.model, model_args.lora_path)
    return model
